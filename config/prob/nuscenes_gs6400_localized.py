...
model = dict(
    ...
    encoder=dict(
        ...
        deformable_model=dict(
            type='LocalizedDeformableFeatureAggregation',  # Using our new localized attention
            embed_dims=embed_dims,
            residual_mode="none",
            # Parameters specific to localized attention
            window_size=15,  # Window size in pixels
            max_points_per_gaussian=8,  # Max number of feature points to attend to per Gaussian
            use_depth_for_window=True,  # Scale window size based on depth
            min_window_ratio=0.5,  # Minimum window size ratio for close objects
            max_window_ratio=2.0,  # Maximum window size ratio for far objects
            kps_generator=dict(
                embed_dims=embed_dims,
